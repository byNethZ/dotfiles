  ...
  (setq-default
   ...
   '(
     ...
     (typescript :variables
                 typescript-backend 'lsp) ; Usa LSP para la inteligencia de código
     (php :variables
          php-backend 'lsp-intelephense) ; Usa LSP con Intelephense
     (html :variables
           html-backend 'lsp) ; Habilita LSP para HTML, útil para Tailwind
     prettier
     sql
     docker
...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   dotspacemacs-completion-system 'helm
...
before packages are loaded."
  ...
  (add-hook 'prisma-mode-hook #'lsp) ; Le dice al modo Prisma que active el LSP

  ;; Configuración para GitHub Copilot
  ;; (Asegúrate de haber iniciado sesión con Copilot en la CLI primero: `github.copilot.auth`)
  (add-hook 'prog-mode-hook 'copilot-mode)

  ;; Habilita el formateo con Prettier al guardar para archivos JS/TS/JSON
  (with-eval-after-load 'prettier
    (spacemacs/enable-prettier-on-save-for-supported-modes)
    )
